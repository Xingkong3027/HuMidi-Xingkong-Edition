...
### 内容更丰富的社区 MIDI 资源（必须自行核实授权）

以下社区平台通常拥有更丰富、更多样的 MIDI 内容，包括流行音乐、游戏音乐、影视音乐、古典音乐、用户原创作品以及不同用户制作的编曲版本，适合用于查找、试听和研究 MIDI。

但是，这些平台主要收录用户上传或用户创作的内容，各文件的来源和授权状态并不统一。网站提供试听、下载或导出功能，不代表相关 MIDI 文件、编曲或原始音乐作品属于公有领域，也不代表已经获得用于视频、直播、软件发行、GitHub 仓库或公开歌单包的许可。

- [MidiShow](https://www.midishow.com/) — 曲库较大、分类丰富，包含多种风格和不同版本的 MIDI。使用前请检查作品页面中的作者备注、版权标记、来源说明和 MIDI 文件内部元数据，并核实原曲与编曲的版权状态。
- [Online Sequencer – Sequences](https://onlinesequencer.net/sequences) — 提供数量丰富的用户原创序列、转录、编曲和重制内容，并支持在线试听和导出 MIDI。使用其他用户的序列前，应确认序列作者的许可；如果内容基于现有歌曲，还需要核实原始作品的版权和授权。

这些网站的优势是内容数量更多、类型更加丰富，但相关内容不一定采用开放许可。用户必须针对每个文件单独核实授权。

如果页面没有明确标注 Public Domain、CC0、CC BY 或其他适合预期用途的许可，请将其视为“版权状态不明”。必要时应联系上传者、编曲者或相关权利人取得许可，不要仅根据“免费下载”“No Copyright”“可导出 MIDI”等文字判断。
